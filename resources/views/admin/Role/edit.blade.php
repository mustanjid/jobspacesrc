<x-dashboard-layout>
    <section class="bg-white dark:bg-gray-900">
        <div class="mx-auto max-w-2xl px-4 py-2 lg:py-2">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit Role - {{ $getRecord->name }}</h2>
            <form action="/admin/roles/edit/{{ $getRecord->id }}" method="POST">
                @csrf
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            placeholder="Type role name" value="{{ $getRecord->name }}" required="">
                    </div>
                </div>
                        ...
                        <ul
                            ...
                            @foreach ($value['group'] as $group)
                                @php
                                    $checked = '';
                                @endphp
                                @foreach ($getPositionPermission as $permission)
                                    @if ($permission->permission_id == $group['id'])
                                        @php
                                            $checked = 'checked';
                                        @endphp
                                    @endif
                                ...
                                <li
                                    ...
                                    <div class="flex items-center ps-3">
                                        <input id="{{ $group['name'] }}" {{ $checked }} type="checkbox"
                ...
                <button type="submit"
                    class="mb-2 me-2 mt-6 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update
                    role</button>

                    <a class="mb-2 me-2 mt-6 rounded-lg bg-amber-300 px-5 py-2.5 text-sm font-medium text-black hover:bg-cyan-800 " href="/admin/roles">View Role</a>
            </form>
